...
services:
  blink-bridge:
    image: blink-bridge:latest
    build: .
    container_name: blink-bridge
    restart: unless-stopped
    network_mode: host  # Recommended for local MQTT discovery
    volumes:
      - ./config:/config
      - ./app/images:/app/images
    environment:
      - MQTT_BROKER=192.168.0.100  # Replace with your Broker IP
      - MQTT_PORT=1883
      - CONFIG_PATH=/config/blink_config.yaml
```
